...
import pytesseract
pytesseract.pytesseract.tesseract_cmd = "Tesseract-OCR\tesseract.exe" # path of tesseract.exe


class FeatureExtractor:
    
    def __init__(self,file_directory, img_path, result_path, block_model_path, feature_model_path):
        ...
        self.file_directory = file_directory # path were input pdf files are stored
...
#             print('image name:', photo_path)
    ...
    def data_extractor(self, tuple_value):
        time_value, file_name = tuple_value
        time.sleep(time_value)
        self.model_init()
        try:
            photo_path = []
            name_file, ext = os.path.splitext(file_name)
            print('name:', name_file)
            self.output_file_name = f'{self.result_path}/{name_file}.xlsx'
            workbook = xlsxwriter.Workbook(self.output_file_name)
            pdf_path = os.path.join(self.file_directory, file_name).replace('\\', '/')
